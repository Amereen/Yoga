...
function YogaShow() {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:3000/api/v1/yogas/${id}`,
          {
            headers: {
  ...
  const handleMakeExercise = async () => {
    try {
      ...
      toast.success("Added to exercise.", {
        position: toast.POSITION.TOP_RIGHT,
        autoClose: 5000,
      });
      navigate(`/yoga/${id}`);
    ...
    <div className="container mt-5 pb-5">
      <div className="row">
        <div className="col-md-12 text-center show_video">
          ...
          ></iframe>
          <p className="show_yoga_discription">{yoga.description}</p>
        </div>
      </div>
            ...
            <div className="card">
              <div className="card-body">
                ...
                <div className="d-flex justify-content-between">
                  <p className="card-title bg-secondary text-white p-2 rounded">
                    {comment.user_email}
                  </p>
                ...
                {comment.comment_user_id === comment.current_user && (
                  <div className="mt-2">
                    {editCommentId !== comment.id && (
                      <span
                        onClick={() =>
                          handleEditCommentClick(comment.id, comment.content)
                        }
                        className="text-primary mx-3 my-2 text-decoration-underline fw-bold"
                      >
                        Edit
                      </span>
                    )}
